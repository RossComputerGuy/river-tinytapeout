...
logic slave_2_STB;
logic slave_2_WE;
HarborSpiFlashController  flash_1(.clk(clk),.reset(rst_n),.bus_CYC(bus_CYC),.bus_STB(bus_STB),.bus_WE(bus_WE),.bus_ADR(bus_ADR),.bus_DAT_MOSI(bus_DAT_MOSI),.bus_SEL(bus_SEL),.wr_req(),.wr_op(),.wr_addr(),.wr_len(),.wr_data(),.spi_clk(spi_clk),.spi_cs_n(spi_cs_n),.bus_ACK(slave_1_ACK),.bus_DAT_MISO(slave_1_DAT_MISO),.wr_data_index(),.wr_busy(),.wr_done(),.wr_err(),.spi_io(spi_io));
...
RiverCore  river_core(.clk(clk),.reset(rst_n),.dataBus_ACK(dataBus_ACK),.dataBus_DAT_MISO(dataBus_DAT_MISO),.dataBus_CYC(master_CYC),.dataBus_STB(master_STB),.dataBus_WE(master_WE),.dataBus_ADR(master_ADR),.dataBus_DAT_MOSI(master_DAT_MOSI),.dataBus_SEL(master_SEL));
HarborSram_65536  sram_0(.clk(clk),.reset(rst_n),.bus_CYC(CYC),.bus_STB(STB),.bus_WE(WE),.bus_ADR(ADR),.bus_DAT_MOSI(DAT_MOSI),.bus_SEL(SEL),.bus_ACK(slave_0_ACK),.bus_DAT_MISO(slave_0_DAT_MISO));
HarborUart  uart(.clk(clk),.reset(rst_n),.rx(uart_rx),.bus_CYC(slave_2_CYC),.bus_STB(slave_2_STB),.bus_WE(slave_2_WE),.bus_ADR(slave_2_ADR),.bus_DAT_MOSI(slave_2_DAT_MOSI),.bus_SEL(slave_2_SEL),.tx(uart_tx),.interrupt(),.bus_ACK(slave_2_ACK),.bus_DAT_MISO(slave_2_DAT_MISO));
endmodule : tt_um_river_soc
